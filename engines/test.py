...
class TesterBase_Assemble:
    def __init__(self, cfg, model=None, test_loader=None, verbose=False) -> None:
        ...
        if model is None:
            ...
            self.model = model
        self.model2 = self.build_additional_model(cfg.weight_model2)
        self.model3 = self.build_additional_model(cfg.weight_model3)
    ...
    def build_additional_model(self, weight_path):
        model = build_model(self.cfg.model)
        n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
        self.logger.info(f"Num params: {n_parameters}")
        model = create_ddp_model(
        ...
        if os.path.isfile(weight_path):
            ...
            self.logger.info(
                "=> Loaded weight '{}' (epoch {}), {}".format(
                    weight_path, checkpoint["epoch"], load_info
                )
            )
        else:
...
@TESTERS.register_module()
class SemSegTester_Assemble(TesterBase_Assemble):
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                ...
                for i in range(len(fragment_list)):
                    ...
                    with torch.no_grad():
                        pred_part = self.model(input_dict)["seg_logits"]  # (n, k)
                        pred_part2 = self.model2(input_dict)["seg_logits"]  # (n, k)
                        pred_part3 = self.model3(input_dict)["seg_logits"]  # (n, k)
                        
                        pred_all = ( pred_part + pred_part2 + pred_part3 ) / 3.0
                        pred_part = F.softmax(pred_all, -1)
                        if self.cfg.empty_cache:
                            torch.cuda.empty_cache()
                        if use_voxel_broadcast:
                            inv = fragment_list[s_i:e_i][0]["inverse"]
