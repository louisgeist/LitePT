...
@TRANSFORMS.register_module()
class ChromaticAutoContrastv2(object):
    def __init__(self, p=0.2, blend_factor=None):
        self.p = p
        self.blend_factor = blend_factor
...
@TRANSFORMS.register_module()
class MultiBlockGenerator(object):
    def __init__(
        self,
        global_view_num=2,
    ...
    def __call__(self, data_dict):
        ...
        full_point = self.local_transform(copy.deepcopy(data_dict)) # teacher branch aug
        point = self.local_transform(copy.deepcopy(data_dict))  # student branch aug
        # point = copy.deepcopy(full_point) # student branch aug
        z_min = coord[:, 2].min()
        z_max = coord[:, 2].max()
        ...
        major_center = point["coord"][np.random.choice(np.where(center_mask)[0])]
        ...
        # remove overlaps between global view and all local views

        # global_view_index = global_views[0]['index']
        # local_view_index = [local_view['index'] for local_view in local_views]]
        ...
        #     for key in self.view_keys + ('index',):
        ...
        for key in self.view_keys:
            view_dict[key] = full_point[key]
        view_dict['offset'] = view_dict['coord'].shape[0]

        # remove overlaps between global view and all local views

        global_view_index = global_views[0]['index']
        local_view_index = view_dict['local_index']
        context_view_index = global_view_index[~np.isin(global_view_index, local_view_index)]
        
        view_dict['context_index'] = context_view_index
        view_dict['context_offset'] = view_dict['context_index'].shape[0]

        view_dict['student_full_coord'] = point['coord']
        for key in point.keys():
            if key in self.view_keys:
                view_dict[f"context_{key}"] = point[key][context_view_index]
...
@TRANSFORMS.register_module()
class MultiBlockGeneratorPreventOverlap(object):
    def __init__(
        self,
        global_view_num=2,
        global_view_scale=(0.4, 1.0),
...
@TRANSFORMS.register_module()
class MultiBlockGeneratorEarlyFuse(object):
    def __init__(
        self,
        global_view_num=2,
        global_view_scale=(0.4, 1.0),
    ...
    def __call__(self, data_dict):
        ...
        view_dict['student_full_coord'] = point['coord']
        view_dict['student_full_color'] = point['color']
        view_dict['student_full_normal'] = point['normal']
        view_dict['student_full_origin_coord'] = point['origin_coord']
        for key in point.keys():
            if key in self.view_keys:
                view_dict[f"context_{key}"] = point[key][context_view_index]
...
@TRANSFORMS.register_module()
class MultiBlockGeneratorEarlyFuseDiffAug(object):
    def __init__(
        self,
        global_view_num=2,
        global_view_scale=(0.4, 1.0),
    ...
    def __call__(self, data_dict):
        ...
        full_point = self.local_transform(copy.deepcopy(data_dict)) # teacher branch aug
        # point = self.local_transform(copy.deepcopy(data_dict))  # student branch aug
        point = copy.deepcopy(data_dict) # student branch aug
        z_min = coord[:, 2].min()
        z_max = coord[:, 2].max()
        z_min_ = z_min + (z_max - z_min) * self.center_height_scale[0]
        z_max_ = z_min + (z_max - z_min) * self.center_height_scale[1]
        ...
        global_view_index = global_views[0]['index']
        local_view_index = [local_view['index'] for local_view in local_views]
        local_view_offset = np.cumsum(
            [data.shape[0] for data in local_view_index]
        )

        local_view_index = np.concatenate(local_view_index)
        context_view_index = global_view_index[~np.isin(global_view_index, local_view_index)]
        

        # concat context view with each target block view
        context_target_views = []
        for local_view in local_views:
            for key in local_view.keys():
                if key in self.view_keys:
                    local_view[key] = np.concatenate(
                        [point[key][context_view_index], local_view[key]]
                    )
            local_view['index'] = np.arange(context_view_index.shape[0], local_view['coord'].shape[0])
            local_view_mask = np.zeros(local_view['coord'].shape[0])
            local_view_mask[context_view_index.shape[0]:] = 1
            local_view['mask'] = local_view_mask
            context_target_views.append(local_view)

        # augmentation and concat
        view_dict = {}

        for context_target_view in context_target_views:
            # local_view.pop("index")
            context_target_view = self.local_transform(context_target_view)
            for key in self.view_keys + ('index','mask'):
                if f"context_target_{key}" in view_dict.keys():
                    view_dict[f"context_target_{key}"].append(context_target_view[key])
                else:
                    view_dict[f"context_target_{key}"] = [context_target_view[key]]
        view_dict["context_target_offset"] = np.cumsum(
            [data.shape[0] for data in view_dict["context_target_coord"]]
        )

        # update target block's index in the context_target batch
        for d_idx, d in enumerate(view_dict['context_target_index'][1:]):
            d += view_dict["context_target_offset"][d_idx]
        ...
        view_dict["target_offset"] = local_view_offset
        view_dict["target_index_in_full"] = local_view_index
        ...
        # from pointcept.utils.ply import write_ply
        # from pointcept.models.utils import offset2batch, offset2bincount
        # context_target_offset = np.concatenate(
        #         [np.zeros(1),data_dict['context_target_offset']]
        #             ).astype(np.int64)
        # for block_id in range(len(data_dict['context_target_offset'])):
        #     write_ply('viz/check_context_target_{}.ply'.format(block_id),
        #               [data_dict['context_target_coord'][context_target_offset[block_id]: context_target_offset[block_id+1]], 
        #                (data_dict['context_target_color']*255).astype(np.uint8)[context_target_offset[block_id]: context_target_offset[block_id+1]]],
        #                ['x','y','z','R','G','B'])
            
        #     write_ply('viz/check_context_{}.ply'.format(block_id),
        #               [data_dict['context_target_coord'][context_target_offset[block_id]: context_target_offset[block_id+1]], 
        #                (data_dict['context_target_color']*255).astype(np.uint8)[context_target_offset[block_id]: context_target_offset[block_id+1]]],
        #                ['x','y','z','R','G','B'])
            
        # write_ply('viz/check_full.ply',
        #               [data_dict['coord'], 
        #                (data_dict['color']*255).astype(np.uint8)],
        #                ['x','y','z','R','G','B'])
